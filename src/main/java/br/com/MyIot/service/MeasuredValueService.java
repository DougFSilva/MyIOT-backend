...
public class MeasuredValueService {
	...
	public void deleteByTimeRange(String deviceId, DateFilter filter) {
		MeasuringDevice device = findDeviceById(deviceId);
		repository.deleteByTimeRange(device, filter);
		return;
	}

	public List<MeasuredValueDto> findAllByDevice(String deviceId, Integer limit) {
		MeasuringDevice device = findDeviceById(deviceId);
		return repository.findAllByDevice(device, limit)
				.stream()
				.map(measuredValue -> new MeasuredValueDto(measuredValue))
				.toList();
	}

	public List<MeasuredValueDto> findAllByDeviceAndTimeRange(String deviceId, DateFilter filter, Integer limit) {
		MeasuringDevice device = findDeviceById(deviceId);
		return repository.findAllByDeviceAndTimeRange(device, filter, limit)
				.stream()
				.map(measuredValue -> new MeasuredValueDto(measuredValue))
				.toList();
	}
